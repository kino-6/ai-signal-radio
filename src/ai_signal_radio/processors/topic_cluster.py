...
def cluster_items(items: list[NewsItem]) -> list[NewsItem]:
    ...
    for item in items:
        match_index, reason, keywords = _find_cluster(clusters, item)
        if match_index is None:
            clusters.append(
                TopicCluster(
                    cluster_id=_cluster_id(item),
                    label=_cluster_label(item),
                    items=(item,),
                    reason="single_item",
                    keywords=tuple(sorted(_keywords(item))[:8]),
                )
            )
            continue
...
def _find_cluster(
    ...
    for index, cluster in enumerate(clusters):
        reason, keywords = _relation_reason(cluster.items[0], item)
        if reason:
            return index, reason, keywords
    return None, "", ()


def _relation_reason(left: NewsItem, right: NewsItem) -> tuple[str, tuple[str, ...]]:
    left_products = _product_terms(left)
    right_products = _product_terms(right)
...
def _cluster_id(item: NewsItem) -> str:
    payload = f"{item.source}\0{_cluster_label(item)}".encode("utf-8")
    return f"topic-{sha256(payload).hexdigest()[:12]}"


def _cluster_label(item: NewsItem) -> str:
    products = sorted(_product_terms(item))
    if products:
        return products[0]
...
def _product_terms(item: NewsItem) -> set[str]:
    ...
    terms = {
        ...
        if token not in PRODUCT_STOPWORDS and not token.isupper() and not token.endswith("APIs")
    }
    return terms


def _keywords(item: NewsItem) -> set[str]:
