...
`setup.sh` は `uv sync`、`data/` 配下の初期ディレクトリ作成、ネットワーク不要の demo smoke test まで実行します。ローカルサービスの疎通も確認したい場合は、必要なものを起動してから次のように実行します。

```bash
bash scripts/setup.sh --check-ollama --check-voicevox
```

依存同期や smoke test を飛ばしたい場合は `--skip-sync` / `--skip-smoke` を使えます。
