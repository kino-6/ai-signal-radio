...
- 設定ファイルで調整できる透明なヒューリスティック score、source diversity、topic diversity による最終選抜
...
`setup.sh` は `uv sync`、`data/` 配下の初期ディレクトリ作成、ネットワーク不要の demo smoke test まで実行します。`uv` がまだ無い環境では、明示的にインストールも任せられます。

```bash
bash scripts/setup.sh --install-uv
```

ローカルの日次ラジオまで使う場合は、Ollama と VOICEVOX の起動確認、指定 Ollama model の pull もまとめて実行できます。`--start-ollama` と `--start-voicevox` は macOS のアプリ起動を試し、起動済みならそのまま通ります。
...
ローカルサービスの疎通だけ確認したい場合:

```bash
bash scripts/setup.sh --check-ollama --check-voicevox
```

依存同期や smoke test を飛ばしたい場合は `--skip-sync` / `--skip-smoke` を使えます。モデルや接続先は `OLLAMA_MODEL`、`--ollama-model`、`--ollama-url`、`--voicevox-url` で変更できます。
